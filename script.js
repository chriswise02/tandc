document.addEventListener("DOMContentLoaded", () => {
    ...
    function updatePromoInputs(promoType) {
        ...
        if (promoType === "xOff") {
            promoInputsContainer.innerHTML += `
                ...
                <label for="pickup">Pickup Only?</label>
    ...
    function validateInputs(inputs) {
        ...
        document.querySelectorAll(".boolean-field").forEach((field) => {
            if (field.closest("#promoInputs")) { // Ensure the field is within the dynamically added container
                if (!field.value) {
                    errors.push(`Please select "Yes" or "No" for ${field.previousElementSibling.textContent}`);
                }
            }
        });
        ...
        if (document.getElementById("mxLocationExclusive") && inputs.mxLocationExclusive === "Yes" && !mxLocationField) {
            errors.push("If 'Mx/Location Exclusive?' is set to 'Yes', you must fill out 'Mx/Location'.");
        }
    ...
    const promoGenerators = {
        "xOff": (inputs) => {
            ...
            terms += `. Valid on only ${inputs.orders} ${inputs.orders == 1 ? "order" : "orders"}`;
            ...
            if (inputs.alcohol === "No") terms += ` Not valid for the purchase of alcohol.`;
            else terms += ` Alcohol delivery and alcohol sales are permitted in certain states and only to people 21+. Please drink responsibly.`;
            ...
            if (inputs.promoCode) terms += ` Use promo code ${inputs.promoCode.toUpperCase()} to redeem.`;
            if (inputs.dashPassOnly === "Yes") terms += ` Must have an active DashPass subscription.`;
            if (inputs.showInWallet === "Yes") terms += ` Select offer from wallet to redeem.`;
            if (inputs.autoRedeem === "Yes") terms += ` Offer will be automatically applied to order upon checkout.`;
        ...
        "zeroDelivery": (inputs) => {
            ...
            terms += `. Valid only ${inputs.orders} ${inputs.orders == 1 ? "order" : "orders"}`;
            if (inputs.minSubtotal > 0) {
                terms += ` with a minimum subtotal of $${inputs.minSubtotal}`;
            }
            ...
            if (inputs.promoCode) terms += ` Use promo code ${inputs.promoCode.toUpperCase()} to redeem.`;
            if (inputs.dashPassOnly === "Yes") terms += ` Must have an active DashPass account.`;
            if (inputs.showInWallet === "Yes") terms += ` Select offer from wallet to redeem.`;
            if (inputs.autoRedeem === "Yes") terms += ` Offer will be automatically applied to order upon checkout.`;
        ...
        "bogo": (inputs) => {
            ...
            terms += ` Cart must include at least ${inputs.numBogoItems * 2} items labeled as 'Buy 1 get 1 free' menu items.`;
